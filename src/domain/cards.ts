...
export type CardKind = "Number" | "Skip" | "Reverse" | "DrawTwo" | "Wild" | "WildDrawFour";
...
// NEW: alias for alle farvede kort (number + action)
...
export const isWild = (c: Card): c is WildCard => c.kind === "Wild" || c.kind === "WildDrawFour";
...
export function canPlayOn(card: Card, top: Card, activeColor?: Color): boolean {
  ...
  // Hvis top er Wild og der IKKE er valgt farve endnu, må alle farvede kort spilles
  if (isWild(top) && !activeColor) {
